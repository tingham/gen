  ...
  for (int x = 0; x < srcWidth; x++) {
    for (int y = 0; y < srcHeight; y++) {
      if (srcWidth == srcHeight)
      {
        ...
        r = lerp(r, pound(data[x][y] + (data[y][x] * 0.125), 1), 0.2 * b);
        g = lerp(g, pound(data[x][y] + (data[y][x] * 0.125), 1), 0.4 * r);
        b = lerp(g, pound(data[x][y] + (data[y][x] * 0.125), 1), 0.8 * g);
      }
      else {
        // No-hack applicator.
        b = pound(data[x][y], 1);
